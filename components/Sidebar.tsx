...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, storeName, accessiblePages, isMobileOpen, setIsMobileOpen }) => {
  const { logout, currentUser, showToast } = useAppContext();
  ...
  const handleLogout = async () => {
    if (!isOnline) {
        showToast("⚠️ خروج فقط در زمان اتصال به اینترنت مجاز است.");
        return;
    }
      ...
      `}>
        ...
        <div className="mt-auto pt-4 border-t border-gray-200/60">
          ...
          <button
              ...
              className={`w-full flex items-center rounded-xl p-3 text-lg transition-colors ${isOnline ? 'text-slate-700 hover:bg-red-100/70 hover:text-red-600' : 'text-slate-300 cursor-not-allowed'} ${isCollapsed ? 'justify-center' : 'space-x-2 space-x-reverse'}`}
              title={!isOnline ? 'خروج نیاز به اینترنت دارد' : (isCollapsed ? 'خروج' : '')}
            >
              <LogoutIcon />
